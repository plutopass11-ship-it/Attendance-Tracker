...
window.AdminUI = {
    ...
    renderZkteco: async function() {
        await this.renderZktecoStatus();
        await this.renderZktecoMappings();
        await this.renderZktecoDeviceUsers();
    },

    renderZktecoStatus: async function() {
        try {
            const res = await fetch('/api/zkteco/status');
            const status = await res.json();

            const connEl = document.getElementById('zkteco-conn-status');
            const ipEl = document.getElementById('zkteco-device-ip');
            const syncEl = document.getElementById('zkteco-last-sync');
            const logsEl = document.getElementById('zkteco-logs-count');
            ...
            if (ipEl) ipEl.textContent = `${status.ip}:${status.port}`;
            if (syncEl) syncEl.textContent = status.lastSyncAt ? new Date(status.lastSyncAt).toLocaleString() : 'Never';
            if (logsEl) logsEl.textContent = status.logsOnDevice || '0';
        } catch (err) {
            console.error('ZKTeco status render error:', err);
        }
    },

    renderZktecoMappings: async function() {
        const tbody = document.getElementById('zkteco-mappings-tbody');
        if (!tbody) return;
        ...
        try {
            ...
            mappings.forEach(m => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${m.zkteco_uid}</td>
                    <td>${m.zkteco_user_id}</td>
                    <td><span style="color:${statusColor}; font-weight:600;">${m.status}</span></td>
                    <td>
                        <button class="btn-small btn-reject" onclick="window.AdminUI.zktecoUnmapUser('${m.user_id}')">Unlink</button>
                    </td>
                `;
                tbody.appendChild(tr);
            });
        } catch (err) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:var(--danger); padding:24px;">Failed to load mappings.</td></tr>';
        }
    },

    renderZktecoDeviceUsers: async function() {
        const tbody = document.getElementById('zkteco-device-users-tbody');
        if (!tbody) return;
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:var(--text-muted); padding:24px;">Loading...</td></tr>';
        ...
        try {
            ...
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:var(--danger); padding:24px;">Failed to load device users.</td></tr>';
        }
    },

    zktecoTestConnection: async function() {
        const btn = event.target.closest('button');
        const origText = btn.innerHTML;
        btn.innerHTML = '<ion-icon name="refresh-outline" style="vertical-align:middle; margin-right:4px;"></ion-icon> Testing...';
        btn.disabled = true;
        try {
            const res = await fetch('/api/zkteco/test-connection', { method: 'POST' });
            const data = await res.json();
            alert(data.connected ? '✅ Connected to ZKTeco device!' : '❌ Failed to connect.\n\nCheck:\n• Device is powered on\n• Same network (192.168.1.x)\n• Port 4370 not blocked');
            this.renderZktecoStatus();
        } catch (err) {
    ...
    zktecoSyncAttendance: async function() {
        const btn = event.target.closest('button');
        const origText = btn.innerHTML;
        btn.innerHTML = '<ion-icon name="sync-outline" style="vertical-align:middle; margin-right:4px;"></ion-icon> Syncing...';
        btn.disabled = true;
        try {
            const res = await fetch('/api/zkteco/sync-attendance', { method: 'POST' });
            const data = await res.json();
            alert(`Synced ${data.processed || 0} attendance logs.`);
            await Store.syncWithBackend();
            this.renderDashboard();
        } catch (err) {
    ...
    zktecoPushUsers: async function() {
        const btn = event.target.closest('button');
        const origText = btn.innerHTML;
        btn.innerHTML = '<ion-icon name="cloud-upload-outline" style="vertical-align:middle; margin-right:4px;"></ion-icon> Pushing...';
        btn.disabled = true;
        try {
            const res = await fetch('/api/zkteco/push-users', { method: 'POST' });
            const data = await res.json();
            alert(`Pushed ${data.pushed || 0} users to device.\n${data.failed || 0} failed.`);
            this.renderZkteco();
        } catch (err) {
    ...
    zktecoUnmapUser: async function(userId) {
        ...
        try {
            await fetch(`/api/zkteco/unmap/${encodeURIComponent(userId)}`, { method: 'DELETE' });
            this.renderZktecoMappings();
            this.renderZktecoDeviceUsers();
        } catch (err) {
            alert('Unmap error: ' + err.message);
        }
    },

    zktecoMapUserPrompt: async function(zktecoUid, zktecoUserId) {
        const users = this._cachedUsers || [];
        const options = users.map(u => `<option value="${u.id}">${u.name}</option>`).join('');
        const userId = prompt(`Map device user ${zktecoUserId} to Kitsu user:\n\nSelect user ID (copy from list):\n${users.map(u => u.id + ' = ' + u.name).join('\n')}`);
        if (!userId) return;
        try {
            const res = await fetch('/api/zkteco/map-user', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId, zktecoUid, zktecoUserId })
            });
            const data = await res.json();
            if (data.success) {
                this.renderZktecoMappings();
                ...
                alert('Map failed: ' + (data.message || 'Unknown'));
            }
        } catch (err) {
    ...
    zktecoDeleteDeviceUser: async function(uid) {
        if (!confirm(`Delete user ${uid} from device?\n\nThis cannot be undone.`)) return;
        try {
            await fetch(`/api/zkteco/device-user/${uid}`, { method: 'DELETE' });
            this.renderZktecoDeviceUsers();
        } catch (err) {
